...
class MirrorListener(listeners.MirrorListeners):
    ...
    def onUploadComplete(self, link: str, size):
        with download_dict_lock:
            ...
            if SHORTENER is not None and SHORTENER_API is not None and INDEX_URL is not None and SHORTENERLINK_API is not None or None:
                LOGGER.info("SHORTENER and SHORTENER_API found!")
                msg += f'\n\n𝐈𝐧𝐝𝐞𝐱 𝐋𝐢𝐧𝐤: <code>{siurl}</code>\n\n𝐃𝐫𝐢𝐯𝐞 𝐋𝐢𝐧𝐤: <code>{surl}</code>'
                
            if SHORTENER is None and SHORTENER_API is None and INDEX_URL is None and SHORTENERLINK_API is not None:
                LOGGER.info("INDEX URL NULL")
                msg += f'\n\n𝐃𝐫𝐢𝐯𝐞 𝐋𝐢𝐧𝐤: <code>{gshortlink}</code>'

            if SHORTENER is None and SHORTENER_API is None and INDEX_URL is not None and SHORTENERLINK_API is not None:
